...
suites.mdt = function() return result('mdt.boundary', GetResourceState('nexa_mdt') ~= 'missing', 'MDT UI/backend boundary requires live UI test') end
suites.restart = function() return result('restart.manual', true, 'restart requires live FXServer run') end
suites.all = function() local ok = true; for _, name in ipairs({ 'medical', 'police', 'dispatch', 'licenses', 'evidence', 'mdt', 'restart' }) do ok = suites[name]() and ok end; return ok end
