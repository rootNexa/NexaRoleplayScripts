...
foreach ($marker in @('nexa_dispatch_call_types','nexa_dispatch_calls','nexa_dispatch_history','RegisterCallType','CreateDispatchCall','AssignDispatchUnit','SetUnitStatus','CreatePanic','RegisterDispatchAdapter')) { if ($text -notmatch [regex]::Escape($marker)) { throw "Missing dispatch marker $marker" } }
