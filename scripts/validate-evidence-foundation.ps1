...
foreach ($marker in @('nexa_evidence_types','nexa_evidence_records','nexa_evidence_custody','nexa_evidence_analysis','CollectEvidence','PackageEvidence','RecordCustody','RequestAnalysis','CompleteAnalysis','StoreEvidenceLocker')) { if ($text -notmatch [regex]::Escape($marker)) { throw "Missing evidence marker $marker" } }
