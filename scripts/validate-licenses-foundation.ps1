...
foreach ($marker in @('nexa_license_types','nexa_licenses','IssueLicense','RevokeLicense','ValidateLicense','GetLicenseHistory')) { if ($text -notmatch [regex]::Escape($marker)) { throw "Missing licenses marker $marker" } }
