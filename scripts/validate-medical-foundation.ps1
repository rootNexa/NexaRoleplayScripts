...
foreach ($marker in @('nexa_medical_states','nexa_medical_injuries','ApplyInjury','StabilizePatient','RespawnAtHospital','CreateMedicalReport')) { if ($text -notmatch [regex]::Escape($marker)) { throw "Missing medical marker $marker" } }
