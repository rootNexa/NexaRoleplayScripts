...
local function setupPreviewCam()
    DoScreenFadeIn(1000)
    traceVisual('setupPreviewCam SetTimecycleModifier(hud_def_blur)', {
        modifier = 'hud_def_blur'
    })
    SetTimecycleModifier('hud_def_blur')
    SetTimecycleModifierStrength(1.0)
    FreezeEntityPosition(cache.ped, false)
    traceVisual('setupPreviewCam CreateCamWithParams', {
    ...
    traceVisual('setupPreviewCam SetCamActive(true)', {
        camera = previewCam
    })
    ...
    traceVisual('setupPreviewCam RenderScriptCams(true)', {
        camera = previewCam
    })
...
local function destroyPreviewCam()
    ...
    traceVisual('destroyPreviewCam SetTimecycleModifier(default)', {
        modifier = 'default',
        camera = previewCam
    })
    SetTimecycleModifier('default')
    traceVisual('destroyPreviewCam SetCamActive(false)', {
        camera = previewCam
    })
    SetCamActive(previewCam, false)
    DestroyCam(previewCam, true)
    traceVisual('destroyPreviewCam RenderScriptCams(false)', {
        camera = previewCam
    })
...
local function chooseCharacter()
    ...
    traceVisual('chooseCharacter SetTimecycleModifier(default)', {
        modifier = 'default'
    })
    SetTimecycleModifier('default')
    lib.showContext('qbx_core_multichar_characters')
