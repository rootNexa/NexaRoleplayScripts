...
function IsPlayerBanned(source)
    if GetConvar('nexa:qboxCompat', 'false') == 'true' then
        if GetResourceState('nexa_qbox_compat') ~= 'started' then
            lib.print.error('nexa_qbox_compat is not started; skipping Qbox bans.expire fallback by ADR-005.')
            return false
        end

        local ok, banned, message = pcall(function()
            return exports.nexa_qbox_compat:checkBanForSource(source)
        end)
        ...
        lib.print.error(('nexa_qbox_compat ban check failed: %s'):format(banned))
        return false
    end
